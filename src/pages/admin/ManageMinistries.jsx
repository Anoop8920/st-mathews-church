...
export default function ManageMinistries() {
  ...
  const { documents, loading } = useCollection('ministries', { orderBy: 'order', orderDir: 'asc' });
  const { register, handleSubmit, reset, formState: { errors } } = useForm();

  const onSubmit = async (data) => {
    try {
      const payload = { ...data, order: Number(data.order) || 0 };
      if (editingItem) await updateDocument('ministries', editingItem.id, payload);
      else await addDocument('ministries', payload);
  ...
  const handleDelete = async (id) => { if (window.confirm('Delete this ministry?')) await deleteDocument('ministries', id); };
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="card p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-primary-500">{editingItem ? 'Edit Ministry' : 'New Ministry'}</h3>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input {...register('name', { required: 'Name is required' })} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-transparent outline-none" />
              ...
              <div>
                ...
                <input {...register('president')} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-transparent outline-none" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea {...register('description')} rows="3" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-transparent outline-none resize-none" />
            </div>
      ...
      <div className="space-y-2">
          ...
          <div key={item.id} className="card p-4 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                {item.president && <p className="text-sm text-gray-500">President: {item.president}</p>}
              </div>
            </div>
        ...
        {documents.length === 0 && <p className="text-center text-gray-400 py-8">No ministries added yet.</p>}
      </div>
    </div>
  );
}
